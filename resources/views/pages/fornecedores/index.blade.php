...
@section('content')
    <x-common.page-breadcrumb pageTitle="Fornecedores" icon="ecommerce" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteFornecedorNome: '' }"
    >
        <x-common.component-card title="Gerenciar Fornecedores" desc="">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <form method="GET" action="{{ route('fornecedores.index') }}" class="flex w-full gap-3 sm:w-auto">
                    <input
                        type="text"
                        name="search"
                        value="{{ old('search', $search ?? '') }}"
                        placeholder="Buscar por nome, contato, telefone ou e-mail"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full sm:w-96 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                    />
                    <button
                        type="submit"
                        ...
                        <tbody>
                            @forelse ($fornecedores as $fornecedor)
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    ...
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                            {{ optional($fornecedor->created_at)->format('d/m/Y H:i') }}
                                        </p>
                                    </td>
                                    ...
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('fornecedores.destroy', $fornecedor)) }};
                                                    deleteFornecedorNome = {{ Js::from($fornecedor->nome) }};
                                                    $dispatch('open-delete-fornecedor-modal');
                                                "
                                            >
                                                <svg
            ...
            @if ($fornecedores->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $fornecedores->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $fornecedores->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $fornecedores->onFirstPage(),
                            ])
                        >
                            <span>
                        ...
                        <ul class="hidden items-center gap-0.5 sm:flex sm:mx-3">
                            @foreach ($fornecedores->getUrlRange(max(1, $fornecedores->currentPage() - 2), min($fornecedores->lastPage(), $fornecedores->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $fornecedores->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $fornecedores->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $fornecedores->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $fornecedores->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $fornecedores->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-fornecedor-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">
                    Confirmar exclusão
                </h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir o fornecedor <span class="font-medium text-gray-700 dark:text-gray-300"
                        x-text="deleteFornecedorNome"></span>?
                </p>
