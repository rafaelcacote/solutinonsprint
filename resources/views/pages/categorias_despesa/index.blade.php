...
@section('content')
    <x-common.page-breadcrumb pageTitle="Categorias de despesa" icon="forms" />
    ...
    >
        <x-common.component-card title="Gerenciar categorias de despesa" desc="">
            <x-slot:headerActions>
                <a
                    href="{{ route('categorias-despesa.create') }}"
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('categorias-despesa.index') }}" class="flex flex-col gap-3 xl:flex-row xl:items-center">
                    ...
                    <div class="flex gap-3 xl:ml-auto">
                        ...
                        <a
                            href="{{ route('categorias-despesa.index') }}"
                    ...
                    <table class="w-full">
                        <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Descrição</th>
                            ...
                            @forelse ($categoriasDespesa as $categoria)
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <span class="block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ $categoria->nome }}</span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        @if ($categoria->descricao)
                                            <p
                                                class="max-w-md truncate text-gray-700 text-theme-sm dark:text-gray-400"
                                    ...
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <div class="flex items-center gap-3">
                                            <a
                                                href="{{ route('categorias-despesa.edit', $categoria) }}"
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('categorias-despesa.destroy', $categoria)) }};
                                                    deleteCategoriaNome = {{ Js::from($categoria->nome) }};
                                                    $dispatch('open-delete-categoria-despesa-modal');
                                                "
                                            >
                                                <svg
            ...
            @if ($categoriasDespesa->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $categoriasDespesa->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $categoriasDespesa->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $categoriasDespesa->onFirstPage(),
                            ])
                        >
                            <span>
                        ...
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">
                            Página {{ $categoriasDespesa->currentPage() }} de {{ $categoriasDespesa->lastPage() }}
                        </span>

                        <ul class="hidden items-center gap-0.5 sm:flex sm:mx-3">
                            @foreach ($categoriasDespesa->getUrlRange(max(1, $categoriasDespesa->currentPage() - 2), min($categoriasDespesa->lastPage(), $categoriasDespesa->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $categoriasDespesa->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $categoriasDespesa->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $categoriasDespesa->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $categoriasDespesa->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $categoriasDespesa->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-categoria-despesa-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir a categoria de despesa <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteCategoriaNome"></span>?
                </p>
