...
@section('content')
    <x-common.page-breadcrumb pageTitle="Vendas" icon="ecommerce" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteVendaNumero: '' }"
    >
        <x-common.component-card title="Gerenciar vendas" desc="">
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('vendas.index') }}" class="flex flex-col gap-3 xl:flex-row xl:flex-wrap xl:items-end">
                                    ...
                                    <td class="px-4 py-3.5 sm:px-6">
                                        <div class="flex items-center gap-2 sm:gap-3">
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('vendas.destroy', $venda)) }};
                                                    deleteVendaNumero = {{ Js::from($venda->numero) }};
                                                    $dispatch('open-delete-venda-modal');
                                                "
                                            >
                                                <svg
                    ...
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            ...
                            @class([
                                ...
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $vendas->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $vendas->onFirstPage(),
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">
                            Página {{ $vendas->currentPage() }} de {{ $vendas->lastPage() }}
                        </span>

                        <ul class="hidden items-center gap-0.5 sm:mx-3 sm:flex">
                            @foreach ($vendas->getUrlRange(max(1, $vendas->currentPage() - 2), min($vendas->lastPage(), $vendas->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $vendas->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $vendas->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            ...
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $vendas->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $vendas->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-venda-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir a venda <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteVendaNumero"></span>?
                </p>
