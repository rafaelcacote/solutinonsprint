...
@section('content')
    <x-common.page-breadcrumb pageTitle="Itens" icon="ecommerce" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteItemNome: '' }"
    >
        <x-common.component-card title="Gerenciar Itens" desc="">
            <x-slot:headerActions>
                <a
                    href="{{ route('itens.create') }}"
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('itens.index') }}" class="flex flex-col gap-3 xl:flex-row xl:items-center">
                        ...
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                ...
                                @foreach ($categorias as $categoria)
                                    <option value="{{ $categoria->id }}" @selected(($categoriaId ?? '') === (string) $categoria->id)>
                                        {{ $categoria->nome }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="xl:w-[180px]">
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                ...
                                <option value="produto" @selected(($tipo ?? '') === 'produto')>Produto</option>
                                <option value="servico" @selected(($tipo ?? '') === 'servico')>Serviço</option>
                            </select>
                            <span class="absolute z-30 text-gray-500 -translate-y-1/2 pointer-events-none right-4 top-1/2 dark:text-gray-400">
                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex gap-3 xl:ml-auto">
                        ...
                        <a
                            ...
                            class="inline-flex h-11 items-center gap-2 rounded-lg border border-gray-300 bg-white px-5 text-sm font-medium text-gray-700 shadow-theme-xs transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.03]"
                        >
                            <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                ...
                <div class="max-w-full overflow-x-auto custom-scrollbar">
                    <table class="w-full">
                        <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                            <tr>
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Nome</th>
                                ...
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Unidade</th>
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Modo preço</th>
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Preço venda</th>
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Status</th>
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($itens as $item)
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <span class="block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ $item->nome }}</span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <span class="block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ $item->categoria?->nome ?? '-' }}</span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <x-ui.badge color="{{ $item->tipo === 'produto' ? 'info' : 'warning' }}">
                                            {{ $item->tipo === 'produto' ? 'Produto' : 'Serviço' }}
                                        </x-ui.badge>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ $item->unidade_medida }}</p>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <x-ui.badge color="light">{{ ucfirst($item->modo_preco) }}</x-ui.badge>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400">R$ {{ number_format((float) $item->preco_venda, 2, ',', '.') }}</p>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <form method="POST" action="{{ route('itens.toggle-status', $item) }}">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <label class="inline-flex cursor-pointer items-center gap-3">
                                                ...
                                                @if ($item->ativo)
                                                    <span class="text-theme-sm font-medium text-green-600 dark:text-green-400">Ativo</span>
                                                @else
                                                    <span class="text-theme-sm font-medium text-red-600 dark:text-red-400">Inativo</span>
                                                @endif
                                            </label>
                                        </form>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('itens.destroy', $item)) }};
                                                    deleteItemNome = {{ Js::from($item->nome) }};
                                                    $dispatch('open-delete-item-modal');
                                                "
                                            >
                                                <svg
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 sm:px-6 py-8 text-sm text-gray-500 dark:text-gray-400">
            ...
            @if ($itens->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $itens->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $itens->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $itens->onFirstPage(),
                            ])
                        >
                            <span>
                                ...
                                <li>
                                    <a
                                        ...
                                        @class([
                                            ...
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $itens->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $itens->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $itens->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $itens->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $itens->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-item-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir o item <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteItemNome"></span>?
                </p>
