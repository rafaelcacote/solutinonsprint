@php
    $categoriaDefault = old('categoria_despesa_id', $despesa !== null ? $despesa->categoria_despesa_id : '');
    $fornecedorDefault = old('fornecedor_id', $despesa !== null ? $despesa->fornecedor_id : '');
    $descricaoDefault = old('descricao', $despesa !== null ? $despesa->descricao : '');
    $tipoDefault = old('tipo', $despesa !== null ? $despesa->tipo : \App\Models\Despesa::TIPO_VARIAVEL);
    $valorDefault = old('valor', $despesa !== null ? (string) $despesa->valor : '');
    $vencDefault = old('data_vencimento', $despesa !== null && $despesa->data_vencimento ? $despesa->data_vencimento->format('Y-m-d') : '');
    $pagDefault = old('data_pagamento', $despesa !== null && $despesa->data_pagamento ? $despesa->data_pagamento->format('Y-m-d') : '');
    $statusDefault = old('status', $despesa !== null ? $despesa->status : \App\Models\Despesa::STATUS_PENDENTE);
    $formaDefault = old('forma_pagamento_id', $despesa !== null ? $despesa->forma_pagamento_id : '');
    $obsDefault = old('observacoes', $despesa !== null ? $despesa->observacoes : '');
@endphp

<x-common.component-card title="{{ $despesa ? 'Editar despesa' : 'Nova despesa' }}">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div class="md:col-span-2">
                ...
                <input
                    ...
                    placeholder="Ex.: Aluguel, energia, insumos"
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Categoria de despesa<span class="text-error-500">*</span>
                </label>
                <div class="relative z-20 bg-transparent">
                    <select
                        name="categoria_despesa_id"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('categoria_despesa_id') border-red-500 dark:border-red-500 @enderror"
                    >
                        <option value="">Selecione</option>
                        @foreach ($categorias as $cat)
                            <option value="{{ $cat->id }}" @selected((string) $categoriaDefault === (string) $cat->id)>{{ $cat->nome }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Fornecedor</label>
                <div class="relative z-20 bg-transparent">
                    <select
                        name="fornecedor_id"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('fornecedor_id') border-red-500 dark:border-red-500 @enderror"
                    >
                        <option value="">Nenhum</option>
                        @foreach ($fornecedores as $forn)
                            <option value="{{ $forn->id }}" @selected((string) $fornecedorDefault === (string) $forn->id)>{{ $forn->nome }}</option>
                        @endforeach
                    </select>
                    <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                        <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Data de vencimento</label>
                <input
                    type="date"
                    name="data_vencimento"
                    value="{{ $vencDefault }}"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('data_vencimento') border-red-500 dark:border-red-500 @enderror"
                />
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Data de pagamento</label>
                <input
                    type="date"
                    name="data_pagamento"
                    value="{{ $pagDefault }}"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('data_pagamento') border-red-500 dark:border-red-500 @enderror"
                />
                @error('data_pagamento')
                    <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Com status <strong>Pendente</strong>, ao informar a data de pagamento o registro será salvo como <strong>Pago</strong>. Status <strong>Pago</strong> exige data de pagamento.</p>
            </div>
