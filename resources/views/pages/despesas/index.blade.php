...
@section('content')
    <x-common.page-breadcrumb pageTitle="Despesas" icon="tables" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteDespesaDescricao: '' }"
    >
        <x-common.component-card title="Gerenciar despesas" desc="">
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('despesas.index') }}" class="flex flex-col gap-3 xl:flex-row xl:flex-wrap xl:items-end">
                    ...
                    <div class="w-full xl:w-[200px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Categoria</label>
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                ...
                                @foreach ($categoriasFiltro as $cat)
                                    <option value="{{ $cat->id }}" @selected((string) ($categoriaFiltro ?? '') === (string) $cat->id)>{{ $cat->nome }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="w-full xl:w-[200px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Fornecedor</label>
                        <div class="relative z-20 bg-transparent">
                            <select
                                name="fornecedor_id"
                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                            >
                                ...
                                @foreach ($fornecedoresFiltro as $forn)
                                    <option value="{{ $forn->id }}" @selected((string) ($fornecedorFiltro ?? '') === (string) $forn->id)>{{ $forn->nome }}</option>
                                @endforeach
                            </select>
                            <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="w-full xl:w-[150px]">
                        ...
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                <option value="">Todos</option>
                                @foreach (\App\Models\Despesa::TIPO_LABELS as $valor => $rotulo)
                    ...
                    <div class="w-full xl:w-[150px]">
                        ...
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                <option value="">Todos</option>
                                @foreach (\App\Models\Despesa::STATUS_LABELS as $valor => $rotulo)
                    ...
                    <div class="w-full xl:w-[160px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Venc. inicial</label>
                        <input
                            type="date"
                            name="vencimento_inicio"
                            value="{{ old('vencimento_inicio', $vencimentoInicio ?? '') }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                        />
                    </div>

                    <div class="w-full xl:w-[160px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Venc. final</label>
                        <input
                            type="date"
                            name="vencimento_fim"
                            value="{{ old('vencimento_fim', $vencimentoFim ?? '') }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                        />
                    </div>

                    <div class="w-full xl:w-[160px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Pag. inicial</label>
                        <input
                            type="date"
                            name="pagamento_inicio"
                            value="{{ old('pagamento_inicio', $pagamentoInicio ?? '') }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                        />
                    </div>

                    <div class="w-full xl:w-[160px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Pag. final</label>
                        <input
                            type="date"
                            name="pagamento_fim"
                            value="{{ old('pagamento_fim', $pagamentoFim ?? '') }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                        />
                    </div>
                    ...
                    <table class="w-full min-w-[1200px]">
                        <thead class="border-y border-gray-100 bg-gray-50 px-6 py-3.5 dark:border-white/[0.05] dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400 sm:px-6">Fornecedor</th>
                                ...
                                <th class="px-4 py-3 text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400 sm:px-6">Vencimento</th>
                                <th class="px-4 py-3 text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400 sm:px-6">Pagamento</th>
                            ...
                            @forelse ($despesas as $despesa)
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    <td class="px-4 py-3.5 sm:px-6">
                                        <span class="block text-theme-sm font-medium text-gray-800 dark:text-gray-200">{{ \Illuminate\Support\Str::limit($despesa->descricao, 48) }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400 sm:px-6">
                                        {{ optional($despesa->created_at)->format('d/m/Y H:i') }}
                                    </td>
                                    <td class="px-4 py-3.5 sm:px-6">
                                        <div class="flex items-center gap-2 sm:gap-3">
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('despesas.destroy', $despesa)) }};
                                                    deleteDespesaDescricao = {{ Js::from(\Illuminate\Support\Str::limit($despesa->descricao, 80)) }};
            ...
            @if ($despesas->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $despesas->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $despesas->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $despesas->onFirstPage(),
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <ul class="hidden items-center gap-0.5 sm:mx-3 sm:flex">
                            @foreach ($despesas->getUrlRange(max(1, $despesas->currentPage() - 2), min($despesas->lastPage(), $despesas->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $despesas->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $despesas->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $despesas->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $despesas->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $despesas->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-despesa-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir a despesa <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteDespesaDescricao"></span>?
                </p>
