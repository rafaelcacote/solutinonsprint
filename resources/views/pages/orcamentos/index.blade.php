...
@section('content')
    <x-common.page-breadcrumb pageTitle="Orçamentos" icon="task" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteOrcamentoNumero: '' }"
    >
        <x-common.component-card title="Gerenciar orçamentos" desc="">
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('orcamentos.index') }}" class="flex flex-col gap-3 xl:flex-row xl:flex-wrap xl:items-end">
                    <div class="min-w-0 flex-1 xl:min-w-[220px] xl:max-w-[280px]">
                        <label class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">Busca</label>
                        <input
                            type="text"
                            name="search"
                    ...
                    <table class="w-full min-w-[1100px]">
                        <thead class="border-y border-gray-100 bg-gray-50 px-6 py-3.5 dark:border-white/[0.05] dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400 sm:px-6">Validade</th>
                            ...
                            @forelse ($orcamentos as $orcamento)
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    <td class="px-4 py-3.5 sm:px-6">
                                        <span class="block text-theme-sm font-medium text-gray-800 dark:text-gray-200">{{ $orcamento->numero }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400 sm:px-6">
                                        {{ $fmt($orcamento->subtotal) }}
                                    </td>
                                    <td class="px-4 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400 sm:px-6">
                                        {{ $fmt($orcamento->desconto) }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3.5 sm:px-6">
                                        <div class="flex items-center gap-2 sm:gap-3">
                                            <a
                                                href="{{ route('orcamentos.show', $orcamento) }}"
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('orcamentos.destroy', $orcamento)) }};
                                                    deleteOrcamentoNumero = {{ Js::from($orcamento->numero) }};
                                                    $dispatch('open-delete-orcamento-modal');
                                                "
                                            >
                                                <svg
            ...
            @if ($orcamentos->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $orcamentos->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $orcamentos->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $orcamentos->onFirstPage(),
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <ul class="hidden items-center gap-0.5 sm:flex sm:mx-3">
                            @foreach ($orcamentos->getUrlRange(max(1, $orcamentos->currentPage() - 2), min($orcamentos->lastPage(), $orcamentos->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $orcamentos->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $orcamentos->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $orcamentos->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $orcamentos->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $orcamentos->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-orcamento-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir o orçamento <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteOrcamentoNumero"></span>?
                </p>
