...
@section('content')
    <x-common.page-breadcrumb pageTitle="Formas de pagamento" icon="forms" />
    ...
    <div
        class="space-y-6"
        x-data="{ deleteAction: '', deleteFormaNome: '' }"
    >
        <x-common.component-card title="Gerenciar formas de pagamento" desc="">
            ...
            <div class="flex flex-col gap-4">
                <form method="GET" action="{{ route('formas-pagamento.index') }}" class="flex flex-col gap-3 xl:flex-row xl:items-center">
                    ...
                    <div class="xl:w-[220px]">
                        <div class="relative z-20 bg-transparent">
                            ...
                            >
                                ...
                                @foreach (\App\Models\FormaPagamento::TIPOS_LABELS as $valor => $rotulo)
                                    <option value="{{ $valor }}" @selected(($tipoFiltro ?? '') === $valor)>
                                        {{ $rotulo }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <table class="w-full">
                        <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Data de cadastro</th>
                        ...
                        <tbody>
                            @forelse ($formasPagamento as $forma)
                                @php
                                    ...
                                    $labelTipo = \App\Models\FormaPagamento::TIPOS_LABELS[$forma->tipo] ?? $forma->tipo;
                                @endphp
                                <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <span class="block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ $forma->nome }}</span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <x-ui.badge color="{{ $corTipo }}">{{ $labelTipo }}</x-ui.badge>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        @if ($forma->ativo)
                                            <x-ui.badge color="success">Ativo</x-ui.badge>
                                        @else
                                            <x-ui.badge color="error">Inativo</x-ui.badge>
                                        @endif
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                            {{ optional($forma->created_at)->format('d/m/Y H:i') }}
                                        </p>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3.5">
                                        <div class="flex items-center gap-3">
                                            <a
                                                href="{{ route('formas-pagamento.edit', $forma) }}"
                                            ...
                                            <button
                                                ...
                                                @click="
                                                    deleteAction = {{ Js::from(route('formas-pagamento.destroy', $forma)) }};
                                                    deleteFormaNome = {{ Js::from($forma->nome) }};
                                                    $dispatch('open-delete-forma-pagamento-modal');
                                                "
                                            >
                                                <svg
            ...
            @if ($formasPagamento->hasPages())
                <div class="border-t border-gray-100 p-4 sm:p-6 dark:border-gray-800">
                    <div class="flex items-center justify-between gap-2 px-0 py-0 sm:justify-normal">
                        <a
                            href="{{ $formasPagamento->previousPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $formasPagamento->onFirstPage() === false,
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $formasPagamento->onFirstPage(),
                            ])
                        >
                            <span>
                        ...
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">
                            Página {{ $formasPagamento->currentPage() }} de {{ $formasPagamento->lastPage() }}
                        </span>

                        <ul class="hidden items-center gap-0.5 sm:flex sm:mx-3">
                            @foreach ($formasPagamento->getUrlRange(max(1, $formasPagamento->currentPage() - 2), min($formasPagamento->lastPage(), $formasPagamento->currentPage() + 2)) as $page => $url)
                                <li>
                                    <a
                                        href="{{ $url }}"
                                        @class([
                                            'flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium',
                                            'bg-brand-500 text-white hover:bg-brand-500 hover:text-white' => $page === $formasPagamento->currentPage(),
                                            'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white' => $page !== $formasPagamento->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                        ...
                        <a
                            href="{{ $formasPagamento->nextPageUrl() ?? '#' }}"
                            @class([
                                'flex items-center gap-2 rounded-lg border p-2 sm:p-2.5 shadow-theme-xs',
                                'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200' => $formasPagamento->hasMorePages(),
                                'pointer-events-none border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-600' => $formasPagamento->hasMorePages() === false,
                            ])
                        >
                            <span>
                                <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <x-ui.modal @open-delete-forma-pagamento-modal.window="open = true" :isOpen="false" class="max-w-[520px]">
            <div class="w-full p-6 sm:p-8">
                <h4 class="text-xl font-semibold text-gray-800 dark:text-white/90">Confirmar exclusão</h4>
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                    Tem certeza que deseja excluir a forma de pagamento <span class="font-medium text-gray-700 dark:text-gray-300" x-text="deleteFormaNome"></span>?
                </p>
